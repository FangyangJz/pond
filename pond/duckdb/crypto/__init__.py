...
class CryptoDB(DuckDB):
    ...
    def __init__(self, db_path: Path):
        ...
        self.path_crypto_list = [
            ...
            self.path_crypto_agg_trades, self.path_crypto_agg_trades_origin,
        ]
        ...
        for f in (pbar := tqdm(self.path_crypto_agg_trades_origin.glob('*.csv'))):
            ...
            if f.stem not in agg_trades_list:
                (self.con.sql(
                 ...
                 .write_parquet(str(self.path_crypto_agg_trades / f'{f.stem}.parquet'), compression=self.compress))
