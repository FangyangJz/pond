...
def update_res_dict(
    ...
    for idx, row in stock_df.iterrows():
        ...
        df_qfq = make_fq(
            code=stock_code, code_df=stock_daily_df, gbbq_df=gbbq_df, cw_dict=cw_dict
        )
...
def qfq_acc(
    ...
    pbar.set_description(
        f"Function qfq_acc(), total {len(stock_basic_df)}, step {step}"
    )
...
def make_fq(
    ...
    gbbq_df = gbbq_df.assign(
        date=pd.to_datetime(gbbq_df["权息日"], format="%Y%m%d")
    )  # 添加date列，设置为datetime64[ns]格式
    gbbq_df.set_index(
        "date", drop=True, inplace=True
    )  # 设置权息日为索引  (字符串表示的日期 "19910101")
