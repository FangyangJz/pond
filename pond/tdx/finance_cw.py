...
def get_local_cw_file_list(ext_name=".dat") -> List[Path]:
    ...
    return [
        f
        for f in cw_path.glob("*" + ext_name)
        if len(f.stem) == 12 and f.stem[:4] == "gpcw"
    ]
...
def get_history_financial_df(filepath: Union[Path, str]) -> pd.DataFrame:
    ...
    with open(filepath, "rb") as cw_file:
        ...
        results = []
        for stock_item in struct.iter_unpack(
            "<6s1c1L", cw_file.read(max_count * struct.calcsize("<6s1c1L"))
        ):
...
def get_cw_dict_acc(pool: PoolType = None) -> Dict[str, pd.DataFrame]:
    ...
    step = int(
        len(local_cw_file_list) / (4 * pool._processes)
    )  # tune coe 4 get best speed
    pbar = tqdm(total=int(len(local_cw_file_list) / step))
    pbar.set_description(
        f"Function get_cw_dict_acc() total {len(local_cw_file_list)}, step {step}"
    )
    ...
    for i in range(0, len(local_cw_file_list), step):
        list_r = (
            len(local_cw_file_list) if i + step > len(local_cw_file_list) else i + step
        )
        pool.apply_async(
            update_cw_dict,
...
if __name__ == "__main__":
    ...
    logger.success(
        f"{os.cpu_count() - 1} process time cost: {time.perf_counter() - start_time:.4f}s"
    )
