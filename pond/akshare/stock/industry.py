...
def gen_industry_index(df_snapshot:pl.LazyFrame, path:Path, compress='zstd'):
    ...
    for industry in tqdm(df_industries, desc="Generating industry index..."):
        ...
        df_industry_snapshot = df_snapshot.join(df_industry.lazy(), on="code").with_columns([
                pl.when(pl.col("dchgpct") > 0).then(1).when(pl.col("dchgpct") < 0).then(-1).otherwise(0).alias("pn_count")
                ...
                (pl.col("pn_count").sum() / len(df_industry)).alias("industry_pn_ratio")
            ]).with_columns([
