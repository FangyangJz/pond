...
def get_stock_hist_df(
    ...
    try:
        ...
        if hfq_df.empty:
            logger.error(
                f"[stock_zh_a_hist]{code} {name} hfq dataframe is empty. Maybe limited."
            )
            return pd.DataFrame()
        ...
        nfq_df = stock_zh_a_hist(
            ...
            end_date=end_date,
            adjust="",
        )[nfq_df_columns_dict.keys()].rename(columns=nfq_df_columns_dict)
...
def update_res_dict_thread(
    df: pd.DataFrame, start_date_str: str, end_date_str: str, res_dict: dict
):
    ...
    for idx, item in tqdm(df.iterrows(), desc="Get stock hist df from akshare"):
        ...
        t = threading.Thread(
            target=get_stock_hist_df,
            args=(res_dict, code, name, start_date_str, end_date_str),
        )
        t.start()
        t_list.append(t)
    ...
    logger.success(
        f"Items: {len(res_dict)}/{len(df)}. Threading cost: {time.perf_counter() - start_time:.2f}s"
    )
