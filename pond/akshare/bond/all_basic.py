...
from pond.utils.code2code import MARKET_SH, MARKET_SZ, JUEJIN_MARKET_MAP, trans_to_juejin_code
...
def get_bond_basic_df(data_delist_status: str = "include") -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    bond_zh_cov_df.loc[bond_zh_cov_df['债券代码'].str.contains("^12\d{4}"), "market"] = JUEJIN_MARKET_MAP[MARKET_SZ]
    bond_zh_cov_df.loc[bond_zh_cov_df['债券代码'].str.contains("^11\d{4}"), "market"] = JUEJIN_MARKET_MAP[MARKET_SH]

    bond_cb_redeem_jsl_df, redeem_df = get_bond_cb_redeem_jsl_df()

    if data_delist_status == "include":
        bond_zh_cov_df = bond_zh_cov_df.merge(bond_cb_redeem_jsl_df, on=["债券代码", '正股代码'], how="outer")
    elif data_delist_status == "exclude":
        bond_zh_cov_df = bond_zh_cov_df.merge(bond_cb_redeem_jsl_df, on=["债券代码", '正股代码'])
        bond_zh_cov_df = bond_zh_cov_df[bond_zh_cov_df['强赎状态'].str.contains('不强赎')]

    bond_zh_cov_df.drop(
        labels=['名称', "正股名称", "申购日期", "申购代码", "申购上限", "原股东配售-股权登记日",
                "原股东配售-每股配售额", "中签号发布日"],
        axis=1, inplace=True
    )
