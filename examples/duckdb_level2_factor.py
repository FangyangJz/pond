...
def change_datetime_fields(db: StockDB):
    for dir_path in [db.path_stock_level2_trade, db.path_stock_level2_order, db.path_stock_level2_orderbook]:
...
if __name__ == '__main__':
    ...
    view1 = (
        sh_rel1
        .filter("datetime::TIME >= TIME '09:30:00' and datetime::TIME <= TIME '10:00:00'")
        .aggregate(
            (
    ...
    buy_view = view1.filter("BS='B'").select("* EXCLUDE(BS, amt), amt as buy_amt").create_view('buy_view')
    sell_view = view1.filter("BS='S'").select("* EXCLUDE(BS, amt), amt as sell_amt").create_view('sell_view')
    view2 = db.con.sql('select * from buy_view JOIN sell_view USING (jj_code, datetime)').order(
        "jj_code, datetime").create_view('view2')
    view2.show()
